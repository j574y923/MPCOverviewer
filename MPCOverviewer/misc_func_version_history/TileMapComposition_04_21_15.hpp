	...
	bool load(const std::string& tileset, sf::Vector2u tileSize, std::vector<char>(&composition)[384])
	{
		...
		for (unsigned int i = 0; i < 384; ++i)
		{
			...
			if (!composition[i].empty())
			{
				...
				quad_V[0].position = sf::Vector2f((i % 32) * 40 + 84, (i / 32) * 230 + 187);
				quad_V[1].position = sf::Vector2f((i % 32) * 40 + 84 + tileSize.x, (i / 32) * 230 + 187);
				quad_V[2].position = sf::Vector2f((i % 32) * 40 + 84 + tileSize.x, (i / 32) * 230 + 187 + tileSize.y);
				quad_V[3].position = sf::Vector2f((i % 32) * 40 + 84, (i / 32) * 230 + 187 + tileSize.y);
			}
