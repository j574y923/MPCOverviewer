...
public:
	...
	//call undovector's progress function; used after copypaste,etc.
	void updateprogress();
	...
	//correct composition's syntaxual errors (e.g. ab++bc+++q to ab+bc+++++q); only called when saving
	void correctcomposition();
	void outputcomposition(std::string directory);
	...
	void setm_special_filter(std::string special_filter);

	//return string of special_filter which will either be 
	std::string getm_special_filter();

	//return whether needs to be saved or not
	bool getm_saved();
private:
